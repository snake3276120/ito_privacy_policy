...
- Player clicks 0 and 1 to spawn soldier.
  - This is referred as `Bug` to match the setting.
  - The `Bug` generated by this method is also called an `Active Bug` because there are `Passively/Automatically` generated `Bug`, unlocked by upgrades
- A reserve system is implemented:
  - Reserve has a cap, and can be upgraded
  - Reserve is reduced with active `Bug` generation
  - Reserve refills slowly
- Passive `Bugs` can be unlocked via upgrade.
  - When in game, `Passive Bugs` behaves the same as `Active Bugs` but it is automatically spawned, without spending reserve.
  - When offline or outside of the game, `Passive Bug` helps the player earn `Cache`, like most idle clicker games do
- `Bugs` go through the AI's maze with tower array.
- The stage passes when one `Bug` reaches the end of the maze.
- When a `Bug` is killed by the AI's tower, player earns `Cache` (cash, essentially). `Cache` earned is proportional to `Bug's` HP.
- Active Bonus: a `Cache` multiplier. the more `Bugs` on stage, the higher the multiplier, i.e. the more `Cache` the player earns.
- Upgrades. Major upgrades are:
  - Faster and easier `Active Bug` generation
  - `Bug` HP, and speed
  - `Reserve` cap and regeneration rate
  - Unlocks `Passive Bug`
  - Offline `Cache` cap
  - Increase `Active Bonus`
- Three types of tower are implemented, single target, AOE, and slowing down.
- Three types of QTE are implemented:
  - Clicking on an orange `Bug`, triggered by a timer, to earn `Cache`
  - Clicking on 3 red tiles within a given time, triggered by a timer, to temporary shuts down all towers.
  - Clicking a 8-bit sequence of 0 and 1, triggered by use up all reserves and a timer, to gain unlimited reserve for a short period of time.
- Prestige, called `Time Machine`:
  - Resets the game
  - Gives the player permanent bonus to `Bug's` HP and `Cache` multiplier
- Watch Ads to gain bonus, like `Cache` or `Unlimited Reserve` for a short period of time

## Logic and Code Implementation

Since I opened source the game, you can refer to the github repo for all the codes. I will do an introduction here on some of the key features, and maybe summary and some takeaways.
...
### For people who wants to learn coding or game dev

I'd really vote against the idea of `learn coding by making a game in X engine`.